...
if(!function_exists('myPendingProjectChores')){
    function myPendingProjectChores (){
        $pendingProjectChores = auth()->user()->worker->projectChores->where('chore_state_id','=',1);
...
if(!function_exists('myInProgressProjectChores')){
    function myInProgressProjectChores (){
        $inProgressProjectChores = auth()->user()->worker->projectChores->where('chore_state_id','=',3);
...
if(!function_exists('infoHeader')){
    function infoHeader (){
        ...
        if($worker){
            $pendingProjectChores = ProjectChore::orderBy('created_at','desc')->where('worker_id','=',$worker->id)->where('chore_state_id','=',1)->limit(5)->get();
            $inProgressProjectChores = ProjectChore::orderBy('created_at','desc')->where('worker_id','=',$worker->id)->where('chore_state_id','=',3)->limit(5)->get();
            $pausedProjectChores = ProjectChore::orderBy('created_at','desc')->where('worker_id','=',$worker->id)->where('chore_state_id','=',2)->limit(5)->get();
        }
