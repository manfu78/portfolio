@extends('layouts.admin.app')

@section('main_container')
    <div class="main-container container-fluid">
        <div class="page-header">
            @can('documentTypes.create')
                <ul class="nav nav-pills nav-pills-circle mr-3" id="tabs_2" customer="tablist">
                    <li class="nav-item">
                        <a class="me-2 btn btn-sm btn-outline-primary" href="{{ route('admin.documentTypes.create') }}">
                            <span class="nav-link-icon d-block"><i class="fe fe-file-plus"></i> {{ trans('messages.DocumentType.NewDocumentType') }}</span>
                        </a>
                    </li>
                </ul>
            @else
                <h1 class="page-title">&nbsp;</h1>
            @endcan
            <div>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><small>{{ trans('messages.Home.Home') }}</small></a></li>
                    <li class="breadcrumb-item active"><small>{{ trans('messages.DocumentType.DocumentTypes') }}</small></li>
                </ol>
            </div>
        </div>
        <div class="row row-sm">
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header bg-info-transparent p-2">
                        <span class="fw-bold"><i class="fa-solid fa-grip-vertical"></i></span>&nbsp;{{ trans('messages.DocumentType.DocumentTypes') }}
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="documentTypes_table" class="table table-bordered text-nowrap border-bottom">
                                <thead>
                                    <tr>
                                        <th class="border-bottom-0"><small>id</small></th>
                                        <th class="border-bottom-0"><small>{{ trans('messages.Name') }}</small></th>
                                        <th class="border-bottom-0"></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($documentTypes as $documentType)
                                        <tr>
                                            <td class="py-1" style="width: 10px;">{{ $documentType->id }}</td>
                                            <td class="py-1"><span class="fw-bold">{{ $documentType->type }}</span></td>
                                            <td class="py-1" style="width: 60px;text-align: right;">
                                                @can('documentTypes.edit')
                                                    <a href="{{ route('admin.documentTypes.edit', $documentType) }}"
                                                        class="btn text-default btn-sm py-0"
                                                        data-bs-placement="top"
                                                        data-bs-toggle="tooltip"
                                                        title="{{ trans('messages.Edit') }}">
                                                        <i class="fe fe-edit"></i>
                                                    </a>
                                                @else
                                                    <button type="button" class="btn text-default btn-sm py-0 disabled" disabled>
                                                        <i class="fe fe-edit"></i>
                                                    </button>
                                                @endcan
                                                @can('documentTypes.destroy')
                                                    <a class="modal-effect btn text-danger btn-sm py-0"
                                                        data-bs-effect="effect-flip-horizontal" data-bs-toggle="modal" href="#modalEliminar"
                                                        data-name="{{ $documentType->type }}"
                                                        data-route="{{ route('admin.documentTypes.destroy',$documentType->id) }}">
                                                        <div data-bs-placement="top" data-bs-toggle="tooltip" title="{{ trans('messages.Delete') }}">
                                                            <i class="fe fe-trash"></i>
                                                        </div>
                                                    </a>
                                                @else
                                                    <button type="button" class="btn btn-sm btn-icon btn-danger disabled" disabled>
                                                        <i class="fe fe-edit"></i>
                                                    </button>
                                                @endcan
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- End Row -->
    </div>
    @can('documentTypes.destroy')
        @include('layouts.admin.modal.delete-reg-html')
    @endcan
@endsection

@section('sectiontitle') {{ trans('messages.DocumentType.DocumentTypes') }} @endsection
@section('pagetitle') {{ trans('messages.DocumentType.DocumentTypes') }} @endsection

@section('page_css') @endsection


@section('scripts_js')
    @include('layouts.admin.scripts_js.datatable')
@endsection

@section('scripts')
    <script>
        $(function () {
            $("#documentTypes_table").DataTable({
                "columnDefs": [
                    { orderable: false, targets: [2] },
                    { responsivePriority: 1, targets: 0 },
                    { responsivePriority: 1, targets: 2 },
                ],
                "order": [[0, 'asc']],
                "responsive": true,
                "lengthChange": true,
                "autoWidth": false,
                "language": {
                    "lengthMenu": "Mostrar _MENU_ ",
                    "zeroRecords": "No se han encontrado registros",
                    "info": "P&aacute;gina _PAGE_ de _PAGES_",
                    "infoEmpty": "Mostrando 0 to 0 of 0 registros",
                    "emptyTable": "No hay datos en la tabla",
                    "infoEmpty": "Sin registros",
                    "loadingRecords": "Cargando...",
                    "infoFiltered": "(filtrados de _MAX_ registros)",
                    "processing": "Procesando...",
                    "search": "Busc: ",
                    "thousands": ".",
                    "decimal": ",",
                    "paginate": {
                        "first": "Primero",
                        "last": "&Uacute;ltimo",
                        "next": ">",
                        "previous": "<"
                    }
                },

            }).buttons().container().appendTo('#documentTypes_table_wrapper .col-md-6:eq(0)');
        });
    </script>
     @can('documentTypes.destroy')
        @include('layouts.admin.modal.delete-reg-scritp')
    @endcan
@endsection
