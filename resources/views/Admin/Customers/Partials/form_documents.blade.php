<div class="row">
    <div class="col">
        <div class="card" id="customer_documents">
            ...
            <div class="card-body p-2">
                <div class="row mb-4">
                    <div class="col-12">
                        <div class="text-dark">
                            <h5 class=" mb-1">
                                <i class="fa-solid fa-building"></i>:&nbsp;{{ $customer->name }}
                            </h5>
                            <h5 class="text-muted m-0"><small><i class="fa-solid fa-envelope"></i>:</small>&nbsp;<a href="mailto:{{ $customer->email  }}">{{ $customer->email }}</a></h5>
                            <h5 class="text-muted m-0"><small><i class="fa-solid fa-phone"></i>:</small>&nbsp;<a href="tel:{{ $customer->phone }}">{{ $customer->phone }}</a></h5>
                        </div>
                    </div>
                </div>
                <div class="row">
                        ...
                        <table id="documents_table" class="table table-bordered border-bottom">
                            ...
                            <tbody>
                                @foreach($customer->documents as $customerDocument)
                                    <tr>
                                        <td class="py-1 fw-bold" style="width: 10px;">{{ $customerDocument->id }}</td>
                                        <td class="py-1" style="width: 50px;"><span>{{ date('m/d/Y', strtotime($customerDocument->created_at)) }}</span></td>
                                        <td class="py-1"><span>{{ $customerDocument->name }}</span></td>
                                        <td class="py-1"><span>{{ $customerDocument->documentType->type }}</span></td>
                                        <td class="py-1 text-center">
                                            <a href="{{ Storage::url( $customerDocument->file) }}" target="_blank">
                                        ...
                                        <td class="py-1 text-nowrap" style="width: 30px;text-align: right;">
                                            @can('partials.edit')
                                                <a class="modal-effect btn text-danger btn-sm py-0"
                                                    ...
                                                    data-name="{{ $customerDocument->name }}"
                                                    data-route="{{ route('admin.customers.deleteDocument',$customerDocument) }}">
